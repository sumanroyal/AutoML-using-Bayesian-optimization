...
def automl_bayes_pipeline(df,scoring_method):
    ...
    if len(categorical)!=0 and len(discrete)!=0 and len(continuous)!=0:
        ...
        mean_imputer.fit(X_train_copy)
        # transform the data
        X_train_copy = mean_imputer.transform(X_train_copy)
        X_test_copy = mean_imputer.transform(X_test_copy)

        median_imputer = MeanMedianImputer(imputation_method='median',variables=discrete)
        median_imputer.fit(X_train_copy)
        X_train_copy = median_imputer.transform(X_train_copy)
        X_test_copy = median_imputer.transform(X_test_copy)

        imputer = imputation.CategoricalImputer(imputation_method='frequent', variables=categorical)
        ...
        capper.fit(X_train_copy)
        X_train_copy = capper.transform(X_train_copy)
        X_test_copy = capper.transform(X_test_copy)

        encoder = encoding.OneHotEncoder(variables=categorical,drop_last=True)
        ...
        normaliser.fit(X_train_copy)
        X_train_copy = normaliser.transform(X_train_copy)
        X_test_copy = normaliser.transform(X_test_copy)
        pipe_bayes = Pipeline([('rf', RandomForestClassifier(random_state=0))])
        param_grid_bayes = {
            'rf__max_depth': Integer(3, 10),
            'rf__n_estimators': Categorical([50, 100, 150, 200]),
            'rf__min_samples_leaf': Real(0.01, 0.1, prior="log-uniform"),
            'rf__min_samples_split': Categorical(hyp_param['min_samples_split']),
            'rf__max_features': Categorical(hyp_param['max_features']),
            'rf__bootstrap': Categorical(hyp_param['bootstrap'])
        }
        opt = BayesSearchCV(pipe_bayes,[(param_grid_bayes, 100)],cv=5)
        opt.fit(X_train_copy, y_train)
        bayes_param=dict(opt.best_params_)
        ...
        param_grid = [{
            ...
            'rf__max_depth': [bayes_param['rf__max_depth']],
            'rf__n_estimators': [bayes_param['rf__n_estimators']],
            'rf__class_weight': [weights, None],
            'rf__min_samples_leaf': [bayes_param['rf__min_samples_leaf']],
            'rf__min_samples_split': [bayes_param['rf__min_samples_split']],
            'rf__max_features': [bayes_param['rf__max_features']],
            'rf__bootstrap': [bayes_param['rf__bootstrap']]
        },
            {
                # try different feature engineering parameters
                'imputer_discrete__imputation_method': pipeline_param['imputer_discrete__imputation_method'],
